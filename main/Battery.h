...
float get_Vbatt() {
  ...
  for (int i = 0; i < 16; i++) {
    Vbatt +=
        analogReadMilliVolts(BattVoltPin); // Read and accumulate ADC voltage
    delay(1);
  }
  float Vbattf =
      2 * Vbatt / 16 / 1000.0; // Adjust for 1:2 divider and convert to volts
  Serial.print("Vbatt: ");
...
uint8_t estimateSoC(float voltage) {
  ...
  const float voltages[] = {4.20, 4.10, 4.00, 3.90, 3.80, 3.70,
